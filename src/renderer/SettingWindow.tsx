...
export default function SettingWindow(props: Props) {
    ...
    const onSave = () => {
        ...
        if (settings.fontSize !== settingsEdit.fontSize) {
            toastActions.add(
                t('font size changed, effective after next launch')
            );
        }
        props.close();
    };
                ...
                {currentTab === 'ai' && (
                    <Box>
                        <FormControl
                            ...
                            margin="dense"
                        >
                            <InputLabel htmlFor="ai-provider-select">
                                {t('AI Provider')}
                            </InputLabel>
                            <Select
                                label="ai-provider"
        ...
        <Box>
            ...
            <Box sx={{ display: onlyShow(ModelProvider.OpenAI) }}>
                ...
                <>
                    ...
                    />
                    {!settingsEdit.apiHost.match(
                        /^(https?:\/\/)?api.openai.com(:\d+)?$/
                    ) && (
                        ...
                        <>
                            <Trans
                                ...
                                components={[
                                    <a
                                        href={
                                            'https://github.com/THUDM/ChatGLM-6B#api%E9%83%A8%E7%BD%B2'
                                        }
                                        target="_blank"
                                    ></a>,
                                    <a
            ...
            <Box sx={{ display: onlyShow(ModelProvider.ChatboxAI) }}>
                <Box>
                    <TextField
                        ...
                        disabled={
                            premium.premiumActivated || premium.premiumIsLoading
                        }
                        helperText={
                    ...
                    >
                        ...
                        <Typography sx={{ opacity: '1' }}>
                            {t(
                                'Chatbox AI provides an affordable solution to boost productivity with AI'
                            )}
                        </Typography>
                        <Box>
                            {[
...
function TokenConfig(props: {
    ...
    const sliderChangeHandler = (
        key: 'openaiMaxTokens' | 'openaiMaxContextTokens',
        max: number,
        min: number
    ) => {
        return (
...
function wrapDefaultTokenConfigUpdate(settings: Settings): Settings {
    ...
    if (settings.aiProvider === ModelProvider.OpenAI) {
        ...
        settings.openaiMaxContextTokens =
            defaults.settings().openaiMaxContextTokens;
    }
    return { ...settings };
}
