...
export function MessageAttachment(props: {
    ...
    return (
        <div className='flex justify-start items-center mb-2 p-1.5
            border-solid border-slate-400/20 rounded 
            bg-white dark:bg-slate-800'
        >
            {
                filename && (
                    <FileIcon filename={filename} className='w-6 h-6 ml-1 mr-2 text-black dark:text-white' />
                )
            }
            {
                url && (
                    <Link2 className='w-6 h-6 ml-1 mr-2 text-black dark:text-white' strokeWidth={1} />
                )
            }
